 ...
 * @param {Object} args.transaction - Transaction details.
 * @param {number} args.transaction.amount - The amount in the base currency unit (e.g., 30.00 for £30.00).
...
const executeFunction = async ({
  ...
  if (typeof transaction.amount !== 'number' || isNaN(transaction.amount)) {
    return { error: 'The amount field is required and must be a valid decimal number (e.g., 30.00 for £30.00).' };
  }

  // Validate amount is non-negative
  if (transaction.amount < 0) {
    return { error: 'The amount must be a non-negative number.' };
  }

  // Convert to pence (multiply by 100 and round to handle floating point precision)
  const amountInPence = Math.round(transaction.amount * 100);

  // Prepare the minimal request body with all defaults
  const body = {
    transaction: {
